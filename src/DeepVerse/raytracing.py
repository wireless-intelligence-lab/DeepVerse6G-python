...
from DeepVerse.utils import safe_print
from DeepVerse.file_loaders import load_scenario_params_v1, load_scenario_params_v2, load_bs_loc, load_ray_data

def read_raytracing_v2(bs_id, params, user=True):
    bs_id_map = params[c.PARAMSET_COMM]['scenario']['TX_ID_map']
    bs_data_file = os.path.join(params[c.PARAMSET_SCENARIO_FIL], 'TX%i'%bs_id_map[bs_id, 1])
    channel_data = load_ray_data_v2(bs_data_file)
    params[c.PARAMSET_SCENARIO_PARAMS] = load_scenario_params_v2(scenario_files)
    
def read_raytracing_v1(bs_id, params, user=True):
    
    scenario_files = params[c.PARAMSET_SCENARIO_FIL]
    params[c.PARAMSET_SCENARIO_PARAMS] = load_scenario_params_v1(scenario_files)
...
def extract_data_from_ray_v2(ray_data, ids, params):
...
# Extracts the information to a dictionary from the loaded data file
# for the users given in IDs, and with maximum number of paths
def extract_data_from_ray_v1(ray_data, ids, params):
